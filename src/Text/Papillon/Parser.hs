...
p_patList = foldl1 mplus [do p <- dv_patOpM
                          ...
                          do p <- dv_patOpM
                             ...
                             return [p],
                          do return []]
p_opConName = foldl1 mplus [do xx77_77 <- dvCharsM
                               case xx77_77 of
                                   ':' -> return ()
                                   _ -> throwErrorPackratM "':'" "not match pattern"
                               let ':' = xx77_77
...
p_charLit = foldl1 mplus [do xx78_78 <- dvCharsM
                             let c = xx78_78
                             if isAlphaNumOt c
                              then return ()
                          ...
                          do xx79_79 <- dvCharsM
                             ...
                             return (getNTs c)]
p_stringLit = foldl1 mplus [do ddd81_81 <- get
                               ...
                               return (cons c s),
                            do return emp]
p_dq = foldl1 mplus [do xx83_83 <- dvCharsM
                        case xx83_83 of
                            '"' -> return ()
                            _ -> throwErrorPackratM "'\"'" "not match pattern"
                        let '"' = xx83_83
...
p_test = foldl1 mplus [do xx89_89 <- dvCharsM
                          case xx89_89 of
                              '[' -> return ()
                              _ -> throwErrorPackratM "'['" "not match pattern"
                          let '[' = xx89_89
                          ...
                          if True then return () else throwErrorPackratM "True" "not match"
                          return h]
p_hsExpLam = foldl1 mplus [do xx91_91 <- dvCharsM
                              case xx91_91 of
                                  '\\' -> return ()
                                  _ -> throwErrorPackratM "'\\\\'" "not match pattern"
                              let '\\' = xx91_91
...
p_hsExpOp = foldl1 mplus [do l <- dv_hsExpM
                          ...
                          do e <- dv_hsExpM
                             ...
                             if True then return () else throwErrorPackratM "True" "not match"
                             return (getEx e)]
p_hsOp = foldl1 mplus [do xx96_96 <- dvCharsM
                          let c = xx96_96
                          if isOpHeadChar c
                           then return ()
...
p_opTail = foldl1 mplus [do xx104_104 <- dvCharsM
                            let c = xx104_104
                            if isOpTailChar c
                             then return ()
...
p_hsExp = foldl1 mplus [do e <- dv_hsExp1M
                        ...
                        do e <- dv_hsExp1M
                           ...
                           if True then return () else throwErrorPackratM "True" "not match"
                           return (toEx e)]
p_hsExp1 = foldl1 mplus [do xx105_105 <- dvCharsM
                            case xx105_105 of
                                '(' -> return ()
                                _ -> throwErrorPackratM "'('" "not match pattern"
                            let '(' = xx105_105
                            return ()
                            if True then return () else throwErrorPackratM "True" "not match"
                            xx106_106 <- papOptional (foldl1 mplus [do e <- dv_hsExpTypM
                                                                       return ()
                                                                       if True
                                                                        then return ()
                                                                        else throwErrorPackratM "True" "not match"
...
p_hsType = foldl1 mplus [do t <- dv_hsType1M
                         ...
                         do t <- dv_hsType1M
                            ...
                            if True then return () else throwErrorPackratM "True" "not match"
                            return (toTyp t)]
p_hsType1 = foldl1 mplus [do xx121_121 <- dvCharsM
                             case xx121_121 of
                                 '[' -> return ()
                                 _ -> throwErrorPackratM "'['" "not match pattern"
                             let '[' = xx121_121
...
p_integer = foldl1 mplus [do dh <- dv_digitM
                             return ()
                             if True then return () else throwErrorPackratM "True" "not match"
                             xx132_132 <- list (foldl1 mplus [do d <- dv_digitM
                                                                 return ()
                                                                 if True
                                                                  then return ()
                                                                  else throwErrorPackratM "True" "not match"
...
p_alpha = foldl1 mplus [do u <- dv_upperM
                        ...
                        do d <- dv_digitM
                           ...
                           if True then return () else throwErrorPackratM "True" "not match"
                           return d]
p_upper = foldl1 mplus [do xx133_133 <- dvCharsM
                           let u = xx133_133
                           if isUpper u
                            then return ()
                            else throwErrorPackratM "isUpper u" "not match"
                           return u]
p_lower = foldl1 mplus [do xx134_134 <- dvCharsM
                           let l = xx134_134
                           if isLowerU l
                            then return ()
                            else throwErrorPackratM "isLowerU l" "not match"
                           return l]
p_digit = foldl1 mplus [do xx135_135 <- dvCharsM
                           let d = xx135_135
                           if isDigit d
                            then return ()
...
p_spaces = foldl1 mplus [do dv_spaceM >> return ()
                            ...
                            return (),
                         do return ()]
p_space = foldl1 mplus [do xx136_136 <- dvCharsM
                           let s = xx136_136
                           if isSpace s
                            then return ()
                            else throwErrorPackratM "isSpace s" "not match"
                           return (),
                        do xx137_137 <- dvCharsM
                           case xx137_137 of
                               '-' -> return ()
                               _ -> throwErrorPackratM "'-'" "not match pattern"
                           let '-' = xx137_137
                        ...
                        do dv_commentM >> return ()
                           if True then return () else throwErrorPackratM "True" "not match"
                           return ()]
p_notNLString = foldl1 mplus [do ddd139_139 <- get
                                 ...
                                 return (cons c s),
                              do return emp]
p_nl = foldl1 mplus [do xx141_141 <- dvCharsM
                        case xx141_141 of
                            '\n' -> return ()
                            _ -> throwErrorPackratM "'\\n'" "not match pattern"
                        let '\n' = xx141_141
                        ...
                        if True then return () else throwErrorPackratM "True" "not match"
                        return ()]
p_comment = foldl1 mplus [do xx142_142 <- dvCharsM
                             case xx142_142 of
                                 '{' -> return ()
                                 _ -> throwErrorPackratM "'{'" "not match pattern"
                             let '{' = xx142_142
                             ...
                             if True then return () else throwErrorPackratM "True" "not match"
                             ddd144_144 <- get
                             flipMaybe (do xx145_145 <- dvCharsM
                                           case xx145_145 of
                                               '#' -> return ()
                                               _ -> throwErrorPackratM "'#'" "not match pattern"
                                           let '#' = xx145_145
...
p_comments = foldl1 mplus [do dv_notComStrM >> return ()
                           ...
                           do dv_notComStrM >> return ()
                              if True then return () else throwErrorPackratM "True" "not match"
                              return ()]
p_notComStr = foldl1 mplus [do ddd146_146 <- get
                               ...
                               return (),
                            do return ()]
p_comEnd = foldl1 mplus [do xx149_148 <- dvCharsM
                            case xx149_148 of
                                '-' -> return ()
                                _ -> throwErrorPackratM "'-'" "not match pattern"
                            let '-' = xx149_148
