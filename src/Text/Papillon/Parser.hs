...
parse pos___hoge s = d
          where d = Derivs pegFile pragma pragmaStr pragmaEnd moduleDec moduleDecStr whr preImpPap prePeg afterPeg importPapillon varToken typToken pap peg sourceType peg_ definition selection expressionHs expression nameLeaf_ nameLeaf patOp pat pat1 patList opConName charLit stringLit dq pats leaf test hsExpOp hsOp opTail hsExp hsExp1 hsExpTpl hsType hsType1 hsTypeTpl typ variable tvtail integer alpha upper lower digit spaces space notNLString nl comment comments notComStr comEnd char pos___hoge
...
p_patList = foldl1 mplus [do p <- dv_patOpM
                          ...
                          do p <- dv_patOpM
                             ...
                             return [p],
                          do return []]
p_opConName = foldl1 mplus [do xx76_76 <- dvCharsM
                               case xx76_76 of
                                   ':' -> return ()
                                   _ -> throwErrorPackratM "':'" "not match pattern"
                               let ':' = xx76_76
...
p_charLit = foldl1 mplus [do xx77_77 <- dvCharsM
                             let c = xx77_77
                             if isAlphaNumOt c
                              then return ()
                          ...
                          do xx78_78 <- dvCharsM
                             ...
                             return (getNTs c)]
p_stringLit = foldl1 mplus [do ddd80_80 <- get
                               ...
                               return (cons c s),
                            do return emp]
p_dq = foldl1 mplus [do xx82_82 <- dvCharsM
                        case xx82_82 of
                            '"' -> return ()
                            _ -> throwErrorPackratM "'\"'" "not match pattern"
                        let '"' = xx82_82
...
p_test = foldl1 mplus [do xx83_83 <- dvCharsM
                          case xx83_83 of
                              '[' -> return ()
                              _ -> throwErrorPackratM "'['" "not match pattern"
                          let '[' = xx83_83
...
p_hsExpOp = foldl1 mplus [do l <- dv_hsExpM
                          ...
                          do e <- dv_hsExpM
                             ...
                             if True then return () else throwErrorPackratM "True" "not match"
                             return (getEx e)]
p_hsOp = foldl1 mplus [do xx85_85 <- dvCharsM
                          let c = xx85_85
                          if isOpHeadChar c
                           then return ()
...
p_opTail = foldl1 mplus [do xx87_87 <- dvCharsM
                            let c = xx87_87
                            if isOpTailChar c
                             then return ()
...
p_hsExp = foldl1 mplus [do e <- dv_hsExp1M
                        ...
                        do e <- dv_hsExp1M
                           ...
                           if True then return () else throwErrorPackratM "True" "not match"
                           return (toEx e)]
p_hsExp1 = foldl1 mplus [do xx88_88 <- dvCharsM
                            case xx88_88 of
                                '(' -> return ()
                                _ -> throwErrorPackratM "'('" "not match pattern"
                            let '(' = xx88_88
...
p_hsType = foldl1 mplus [do t <- dv_hsType1M
                         ...
                         do t <- dv_hsType1M
                            ...
                            if True then return () else throwErrorPackratM "True" "not match"
                            return (toTyp t)]
p_hsType1 = foldl1 mplus [do xx97_97 <- dvCharsM
                             case xx97_97 of
                                 '[' -> return ()
                                 _ -> throwErrorPackratM "'['" "not match pattern"
                             let '[' = xx97_97
...
p_hsTypeTpl = foldl1 mplus [do t <- dv_hsTypeM
                               return ()
                               if True then return () else throwErrorPackratM "True" "not match"
                               xx103_103 <- dvCharsM
                               let c = xx103_103
                               ...
                               return ()
                               if True then return () else throwErrorPackratM "True" "not match"
                               return (cons (getTyp t) tt),
                            do t <- dv_hsTypeM
                               return ()
                               if True then return () else throwErrorPackratM "True" "not match"
                               return (cons (getTyp t) emp),
                            do return emp]
...
p_integer = foldl1 mplus [do dh <- dv_digitM
                             return ()
                             if True then return () else throwErrorPackratM "True" "not match"
                             xx104_104 <- list (foldl1 mplus [do d <- dv_digitM
                                                                 return ()
                                                                 if True
                                                                  then return ()
                                                                  else throwErrorPackratM "True" "not match"
...
p_alpha = foldl1 mplus [do u <- dv_upperM
                        ...
                        do d <- dv_digitM
                           ...
                           if True then return () else throwErrorPackratM "True" "not match"
                           return d]
p_upper = foldl1 mplus [do xx105_105 <- dvCharsM
                           let u = xx105_105
                           if isUpper u
                            then return ()
                            else throwErrorPackratM "isUpper u" "not match"
                           return u]
p_lower = foldl1 mplus [do xx106_106 <- dvCharsM
                           let l = xx106_106
                           if isLowerU l
                            then return ()
                            else throwErrorPackratM "isLowerU l" "not match"
                           return l]
p_digit = foldl1 mplus [do xx107_107 <- dvCharsM
                           let d = xx107_107
                           if isDigit d
                            then return ()
...
p_spaces = foldl1 mplus [do dv_spaceM >> return ()
                            ...
                            return (),
                         do return ()]
p_space = foldl1 mplus [do xx108_108 <- dvCharsM
                           let s = xx108_108
                           if isSpace s
                            then return ()
                        ...
                        do dv_commentM >> return ()
                           if True then return () else throwErrorPackratM "True" "not match"
                           return ()]
p_notNLString = foldl1 mplus [do ddd111_111 <- get
                                 ...
                                 return (cons c s),
                              do return emp]
p_nl = foldl1 mplus [do xx113_113 <- dvCharsM
                        case xx113_113 of
                            '\n' -> return ()
                            _ -> throwErrorPackratM "'\\n'" "not match pattern"
                        let '\n' = xx113_113
                        ...
                        if True then return () else throwErrorPackratM "True" "not match"
                        return ()]
p_comment = foldl1 mplus [do xx114_114 <- dvCharsM
                             case xx114_114 of
                                 '{' -> return ()
                                 _ -> throwErrorPackratM "'{'" "not match pattern"
                             let '{' = xx114_114
                             ...
                             if True then return () else throwErrorPackratM "True" "not match"
                             ddd116_116 <- get
                             flipMaybe (do xx117_117 <- dvCharsM
                                           case xx117_117 of
                                               '#' -> return ()
                                               _ -> throwErrorPackratM "'#'" "not match pattern"
                                           let '#' = xx117_117
...
p_comments = foldl1 mplus [do dv_notComStrM >> return ()
                           ...
                           do dv_notComStrM >> return ()
                              if True then return () else throwErrorPackratM "True" "not match"
                              return ()]
p_notComStr = foldl1 mplus [do ddd118_118 <- get
                               ...
                               return (),
                            do return ()]
p_comEnd = foldl1 mplus [do xx121_120 <- dvCharsM
                            case xx121_120 of
                                '-' -> return ()
                                _ -> throwErrorPackratM "'-'" "not match pattern"
                            let '-' = xx121_120
