...
p_pats = msum [do p <- dv_patM
                  ...
                  return (id cons p ps),
               do return (id empty)]
p_leaf_ = msum [do xx25_25 <- dvCharsM
                   let _ = xx25_25
                   if id isNot xx25_25
                    then return ()
                    else throwError (strMsg "not match")
...
p_leaf = msum [do t <- dv_testM
               ...
               do v <- dv_variableM
                  return (id right v)]
p_test = msum [do xx26_26 <- dvCharsM
                  let _ = xx26_26
                  if id isOpenBr xx26_26
                   then return ()
                   else throwError (strMsg "not match")
                  h <- dv_hsExpM
                  xx27_27 <- dvCharsM
                  let _ = xx27_27
                  if id isCloseBr xx27_27
                   then return ()
                   else throwError (strMsg "not match")
                  return (id getEx h)]
...
p_alpha = msum [do u <- dv_upperM
                ...
                do d <- dv_digitM
                   return (id d)]
p_upper = msum [do xx28_28 <- dvCharsM
                   let u = xx28_28
                   if id isUpper xx28_28
                    then return ()
                    else throwError (strMsg "not match")
                   return (id u)]
p_lower = msum [do xx29_29 <- dvCharsM
                   let l = xx29_29
                   if id isLowerU xx29_29
                    then return ()
                    else throwError (strMsg "not match")
                   return (id l)]
p_digit = msum [do xx30_30 <- dvCharsM
                   let d = xx30_30
                   if id isDigit xx30_30
                    then return ()
                    else throwError (strMsg "not match")
                   return (id d)]
p_spaces = msum [do _ <- dv_spaceM
                    ...
                    return (id nil),
                 do return (id nil)]
p_space = msum [do xx31_31 <- dvCharsM
                   let _ = xx31_31
                   if id isSpace xx31_31
                    then return ()
                    else throwError (strMsg "not match")
                   return (id nil)]
